...
public class GeneticAlgorithm : MonoBehaviour {
    ...
    Agent GeneCombine(Agent p1, Agent p2, int ratio) {
        ...
        for(int i = 0; i < geneCount; i++) {
            if(i < ratio) {
                a.myC.AddGene(p1.myC.genes[i]);
            }else {
                a.myC.AddGene(p2.myC.genes[i]);
            }

            if (ShouldMutate()) {
                a.myC.genes[i].time += Random.Range(-mutationRate, mutationRate);
                if(a.myC.genes[i].time < 1)
                    a.myC.genes[i].time = 1;

                //LanderActions actionMutate = (LanderActions)Random.Range(0, 3);
                //if ((int)actionMutate == 3)
                //    actionMutate = LanderActions.Thrust;
                //a.myC.genes[i].action = actionMutate;
            }
        }
